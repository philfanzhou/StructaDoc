...
async function load() {
  try { [providers.value, clients.value, administrators.value] = await Promise.all([get('/api/v1/admin/provider-configs'), get('/api/v1/admin/api-clients'), get('/api/v1/admin/administrators')]) }
...
<template>
  <header class="page-header"><div><p class="eyebrow">ADMINISTRATION</p><h1>系统管理</h1><p>管理本地管理员、解析提供方与服务 API 客户端。</p></div></header>
