...
import BlocksResultPanel from '../components/results/BlocksResultPanel.vue'
import DocumentResultPanel from '../components/results/DocumentResultPanel.vue'
import LayoutResultPanel from '../components/results/LayoutResultPanel.vue'
import ResourcesResultPanel from '../components/results/ResourcesResultPanel.vue'
...
async function openDocument(document: DocumentItem) {
  selectedDocument.value = document; runs.value = []; selectedRun.value = undefined; clearRunResult()
  try {
...
function createTabState(): ResultTabState {
  return { runId: undefined, selection: 0, activated: false, loading: false, loaded: false }
}

const tabStates = reactive<Record<ResultTab, ResultTabState>>({
  document: createTabState(),
  blocks: createTabState(),
  layout: createTabState(),
  resources: createTabState(),
})
...
function clearRunResult() {
  ...
  layoutIncomplete.value = false; selectedBlockId.value = undefined; loadingBlocks.value = false
  resultTab.value = 'document'
...
// Every result read below is checked against both the current Run and the selection generation once
// it returns. The generation also makes opening a Run again a fresh lifecycle rather than allowing
// a request from an earlier visit to finish into its new panel.
function stillSelected(runId: string, selection = resultSelection) {
  return selectedRun.value?.id === runId && resultSelection === selection
}

type MetadataRequest<T> = {
  runId: string
  selection: number
  promise: Promise<T>
}

let artifactsLoadedSelection: number | undefined
let assetsLoadedSelection: number | undefined
let pagesLoadedSelection: number | undefined
let artifactsRequest: MetadataRequest<ParseArtifact[]> | undefined
let assetsRequest: MetadataRequest<ParseAsset[]> | undefined
let pagesRequest: MetadataRequest<ParsePage[]> | undefined

async function ensureArtifacts(runId: string, selection: number) {
  if (artifactsLoadedSelection === selection && stillSelected(runId, selection)) return true
  let request = artifactsRequest
  if (!request || request.runId !== runId || request.selection !== selection) {
    request = {
      runId,
      selection,
      promise: get<ParseArtifact[]>(`/api/v1/parse-runs/${runId}/artifacts`),
    }
    artifactsRequest = request
  }

  try {
    const artifactList = await request.promise
    if (!stillSelected(runId, selection)) return false
    artifacts.value = artifactList
    artifactsLoadedSelection = selection
    return true
  } finally {
    if (artifactsRequest === request) artifactsRequest = undefined
  }
}

async function ensureAssets(runId: string, selection: number) {
  if (assetsLoadedSelection === selection && stillSelected(runId, selection)) return true
  let request = assetsRequest
  if (!request || request.runId !== runId || request.selection !== selection) {
    request = {
      runId,
      selection,
      promise: get<ParseAsset[]>(`/api/v1/parse-runs/${runId}/assets`),
    }
    assetsRequest = request
  }

  try {
    const assetList = await request.promise
    if (!stillSelected(runId, selection)) return false
    assets.value = assetList
    assetsLoadedSelection = selection
    return true
  } finally {
    if (assetsRequest === request) assetsRequest = undefined
  }
}

async function ensurePages(runId: string, selection: number) {
  if (pagesLoadedSelection === selection && stillSelected(runId, selection)) return true
  let request = pagesRequest
  if (!request || request.runId !== runId || request.selection !== selection) {
    request = {
      runId,
      selection,
      promise: get<ParsePage[]>(`/api/v1/parse-runs/${runId}/pages`),
    }
    pagesRequest = request
  }

  try {
    const pageList = await request.promise
    if (!stillSelected(runId, selection)) return false
    pages.value = pageList
    pagesLoadedSelection = selection
    return true
  } finally {
    if (pagesRequest === request) pagesRequest = undefined
  }
}
...
async function openRun(run: ParseRun) {
  ...
  await activateTab('document', run.id, selection)
  if (!stillSelected(run.id, selection) || !tabStates.document.loaded) return
  // Without a rendered document there is nothing on the first tab, so open where the result is.
  // Do not move a user who already chose another tab while the Artifact request was in flight.
  if (!hasMarkdown.value && resultTab.value === 'document') {
    resultTab.value = 'blocks'
    await activateTab('blocks', run.id, selection)
  }
}

async function fetchBlocks(reset: boolean, runId: string, selection: number) {
  if (!stillSelected(runId, selection)) return false
  ...
  if (!reset && (loadingBlocks.value || nextBlockSequence.value === undefined)) return false
  ...
  try {
    ...
    const page = await get<ParseBlockList>(`/api/v1/parse-runs/${runId}/blocks?${query}`)
    if (!stillSelected(runId, selection)) return false
    blocks.value = reset ? page.items : [...blocks.value, ...page.items]
    nextBlockSequence.value = page.nextSequence ?? undefined
    return true
  } finally {
    if (stillSelected(runId, selection)) loadingBlocks.value = false
  }
}
...
async function fetchLayoutPage(pageNumber: number, runId: string, selection: number) {
  if (!stillSelected(runId, selection)) return false
  layoutPageNumber.value = pageNumber
  selectedBlockId.value = undefined
  const page = await get<ParseBlockList>(`/api/v1/parse-runs/${runId}/blocks?pageNumber=${pageNumber}&limit=1000`)
  if (!stillSelected(runId, selection) || layoutPageNumber.value !== pageNumber) return false
...
async function activateTab(
  ...
  if (!runId || !stillSelected(runId, selection)) return
  const state = tabStates[tab]
  if (state.runId !== runId || state.selection !== selection) return
  ...
  try {
    let completed = false
    if (tab === 'document') {
      completed = await ensureArtifacts(runId, selection)
    } else if (tab === 'blocks') {
      const [blocksReady, assetsReady] = await Promise.all([
        fetchBlocks(true, runId, selection),
        ensureAssets(runId, selection),
      ])
      completed = blocksReady && assetsReady
    } else if (tab === 'layout') {
      const [pagesReady, assetsReady] = await Promise.all([
        ensurePages(runId, selection),
        ensureAssets(runId, selection),
      ])
      if (!pagesReady || !assetsReady || !stillSelected(runId, selection)) return
      completed = pages.value.length === 0
        || await fetchLayoutPage(pages.value[0].number, runId, selection)
    } else {
      const [assetsReady, artifactsReady] = await Promise.all([
        ensureAssets(runId, selection),
        ensureArtifacts(runId, selection),
      ])
      completed = assetsReady && artifactsReady
    }

    if (!stillSelected(runId, selection) || state.selection !== selection) return
    state.loaded = completed
  } catch (e) {
    if (stillSelected(runId, selection) && state.selection === selection) {
...
async function loadMoreBlocks() {
  ...
  if (!runId) return
  try {
    await fetchBlocks(false, runId, selection)
  } catch (e) {
    if (stillSelected(runId, selection)) message((e as Error).message, true)
  }
}
...
async function openLayoutPage(pageNumber: number) {
  ...
  const selection = resultSelection
  if (!runId) return
  const state = tabStates.layout
  const request = ++layoutPageRequest
  state.loading = true
  try {
    await fetchLayoutPage(pageNumber, runId, selection)
  } catch (e) {
    if (stillSelected(runId, selection) && request === layoutPageRequest) {
      message((e as Error).message, true)
    }
  } finally {
    if (stillSelected(runId, selection)
      && state.selection === selection
      && request === layoutPageRequest) {
      state.loading = false
    }
  }
}
...
async function pollProgress() {
  ...
  // Result data only exists once a run reaches a final status, so the transition opens the Run.
  // Its tab lifecycle then reads only what the visible panel needs rather than every collection.
  ...
  <div class="workspace-grid">
    ...
    <section class="panel detail" v-if="selectedDocument">
      ...
      <template v-if="selectedRun">
        ...
        <DocumentResultPanel
          v-if="tabStates.document.activated"
          v-show="resultTab === 'document'"
          :key="selectedRun.id"
          :run-id="selectedRun.id"
          :artifacts="artifacts"
          :loading="tabStates.document.loading"
          :loaded="tabStates.document.loaded" />
        <BlocksResultPanel
          v-if="tabStates.blocks.activated"
          v-show="resultTab === 'blocks'"
          :key="selectedRun.id"
          :run-id="selectedRun.id"
          :blocks="blocks"
          :assets="assets"
          :loading="tabStates.blocks.loading"
          :loaded="tabStates.blocks.loaded"
          :loading-more="loadingBlocks"
          :has-more="hasMoreBlocks"
          :page-size="blockPageSize"
          @load-more="loadMoreBlocks" />
        <LayoutResultPanel
          v-if="tabStates.layout.activated"
          v-show="resultTab === 'layout'"
          :key="selectedRun.id"
          :run-id="selectedRun.id"
          :pages="pages"
          :page-number="layoutPageNumber"
          :blocks="layoutBlocks"
          :incomplete="layoutIncomplete"
          :selected-block-id="selectedBlockId"
          :assets="assets"
          :loading="tabStates.layout.loading"
          :loaded="tabStates.layout.loaded"
          @select-page="openLayoutPage"
          @select-block="selectedBlockId = $event" />
        <ResourcesResultPanel
          v-if="tabStates.resources.activated"
          v-show="resultTab === 'resources'"
          :key="selectedRun.id"
          :run-id="selectedRun.id"
          :assets="assets"
          :artifacts="artifacts"
