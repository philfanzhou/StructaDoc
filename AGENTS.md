...
- 业务持久化必须支持 SQLite、PostgreSQL 和 MySQL/MariaDB，领域模型、应用层和公共 API 不得依赖单一数据库方言。
- Worker 必须在所有受支持数据库上原子抢占、维护租约并支持崩溃恢复；SQLite 仅承诺单应用实例部署，服务端数据库必须支持多实例 Worker。
...
- 每种受支持数据库必须维护可独立执行的迁移，并通过相同的持久化与任务生命周期契约测试。
