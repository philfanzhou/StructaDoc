...
- 本文件与 `.github` 中的 task/PR 模板共同落实统一项目规范；修改 AI 流程只改本文件，修改项目交付政策时同步更新对应贡献者文档。
...
1. 已明确写出 `## 范围`（或等价的最小修改范围章节）和 `## 验收标准`。
2. 安全或健壮性任务写清保证、不保证和调用方责任；不适用时明确写“无”。
3. 将要改动的实现、测试、契约、配置和部署路径已经完整读过。
4. 已发现的邻近债务已经创建独立 issue 并完成链接；确认没有时明确写“无”。
...
- 先找不变量，再在路径汇合处决定修改点；同一缺陷经多条路径到达同一输出时，测试应覆盖输入集合。
- 行为变更和 bug 修复必须配有与风险相称的自动化测试。
- 成功路径以及适用的失败、取消、安全、认证、资源和并发行为必须与实现一起测试。
...
PR 进入第三轮 review 时，暂停继续写代码，逐个对照 commit 与 issue 的“范围”和“验收标准”。任何
无法追溯到某条验收标准的 commit 都属于范围违规，应从当前 PR 移除并改为独立 issue。

不违反已声明保证的资源整形、纵深防御和内部结构建议，在第三轮之后转为 follow-up issue；违反已声明
保证的问题继续修复，但不得借机吸收邻近债务。
