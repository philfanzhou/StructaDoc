...
namespace StructaDoc.Infrastructure.Resources;
...
public sealed class EfCoreResourceDeletionService(StructaDocDbContext dbContext) : IResourceDeletionService
{
    ...
    private static IQueryable<DocumentEntity> ApplyAccess(IQueryable<DocumentEntity> query, ResourceAccessContext access, DocumentPermissions permission) => access.IsAdministrator || access.IsServiceClient ? query : access.IsInteractiveUser ? query.Where(document => document.OwnerIssuer == access.Issuer && document.OwnerSubject == access.Subject || document.AccessGrants.Any(grant => grant.PrincipalIssuer == access.Issuer && grant.PrincipalSubject == access.Subject && (grant.Permissions & (int)permission) == (int)permission)) : query.Where(_ => false);
    private static IQueryable<ParseRunEntity> ApplyRunAccess(IQueryable<ParseRunEntity> query, ResourceAccessContext access, DocumentPermissions permission) => access.IsAdministrator || access.IsServiceClient ? query : access.IsInteractiveUser ? query.Where(run => run.Document.OwnerIssuer == access.Issuer && run.Document.OwnerSubject == access.Subject || run.Document.AccessGrants.Any(grant => grant.PrincipalIssuer == access.Issuer && grant.PrincipalSubject == access.Subject && (grant.Permissions & (int)permission) == (int)permission)) : query.Where(_ => false);
}
