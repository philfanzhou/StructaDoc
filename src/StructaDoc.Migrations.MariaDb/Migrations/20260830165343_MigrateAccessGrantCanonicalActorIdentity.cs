    ...
    public partial class MigrateAccessGrantCanonicalActorIdentity : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql("""
                ...
                INSERT INTO _structadoc_access_grant_identity_preflight(valid) SELECT 0 WHERE EXISTS (SELECT 1 FROM document_access_grants WHERE OCTET_LENGTH(created_by) > 4096 OR OCTET_LENGTH(principal_issuer) > 512 OR OCTET_LENGTH(principal_subject) > 255);
                ...
                INSERT INTO _structadoc_access_grant_row_format(valid) SELECT CASE WHEN UPPER(ROW_FORMAT) = 'DYNAMIC' THEN 1 ELSE 0 END FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'document_access_grants';
            ...
            migrationBuilder.Sql("""
                UPDATE document_access_grants SET created_by_legacy = CONVERT(created_by USING binary), principal_issuer_binary = CONVERT(principal_issuer USING binary), principal_subject_binary = CONVERT(principal_subject USING binary);
                """);
