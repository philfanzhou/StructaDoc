...
public sealed class EfCoreResourceDeletionService(StructaDocDbContext dbContext) : IResourceDeletionService
{
    ...
    private IQueryable<DocumentEntity> ApplyAccess(IQueryable<DocumentEntity> query, ResourceAccessContext access, DocumentPermissions permission) => access.IsAdministrator ? query : access.HasPrincipalIdentity ? ApplyPrincipalAccess(query, access, DocumentOwnerIdentity.From(access), DocumentOwnerIdentity.CanCompareTextGrant(access, dbContext.Database.ProviderName), permission) : query.Where(_ => false);
    private IQueryable<ParseRunEntity> ApplyRunAccess(IQueryable<ParseRunEntity> query, ResourceAccessContext access, DocumentPermissions permission) => access.IsAdministrator ? query : access.HasPrincipalIdentity ? ApplyPrincipalRunAccess(query, access, DocumentOwnerIdentity.From(access), DocumentOwnerIdentity.CanCompareTextGrant(access, dbContext.Database.ProviderName), permission) : query.Where(_ => false);

    private static IQueryable<DocumentEntity> ApplyPrincipalAccess(IQueryable<DocumentEntity> query, ResourceAccessContext access, DocumentOwnerIdentity owner, bool canCompareTextGrant, DocumentPermissions permission) => !canCompareTextGrant ? query.Where(document => document.OwnerIssuer == owner.Issuer && document.OwnerSubject == owner.Subject) : query.Where(document => document.OwnerIssuer == owner.Issuer && document.OwnerSubject == owner.Subject || document.AccessGrants.Any(grant => grant.PrincipalIssuer == access.Issuer && grant.PrincipalSubject == access.Subject && (grant.Permissions & (int)permission) == (int)permission));
    private static IQueryable<ParseRunEntity> ApplyPrincipalRunAccess(IQueryable<ParseRunEntity> query, ResourceAccessContext access, DocumentOwnerIdentity owner, bool canCompareTextGrant, DocumentPermissions permission) => !canCompareTextGrant ? query.Where(run => run.Document.OwnerIssuer == owner.Issuer && run.Document.OwnerSubject == owner.Subject) : query.Where(run => run.Document.OwnerIssuer == owner.Issuer && run.Document.OwnerSubject == owner.Subject || run.Document.AccessGrants.Any(grant => grant.PrincipalIssuer == access.Issuer && grant.PrincipalSubject == access.Subject && (grant.Permissions & (int)permission) == (int)permission));
}
