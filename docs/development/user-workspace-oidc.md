...
Result tabs load and mount on first use. Opening a Run initially reads Artifact metadata to decide whether Document or Structure is the useful first tab. Structure then reads its first Block page and Asset metadata, Layout reads Page metadata and the selected page's Blocks, and Resources reuses or reads Asset and Artifact metadata. A tab that has never been opened has no panel DOM, iframe, or image elements. Once opened, its component stays mounted while another tab is visible, preserving selection and scroll position without another metadata read; selecting a different Run unmounts every old panel and resets all tab state. Applicable Block, Layout, and Resource images also use native lazy loading, so creating a visible panel does not require eagerly downloading every Asset body.
