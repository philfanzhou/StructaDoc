...
- Both identity parts compare case-sensitively. Document owners and access-grant
  principals use the canonical one-byte ASCII BLOB/bytea/varbinary mapping, so every
  accepted byte, including NUL, compares directly.
...
have full document permission. Explicit grants target another canonical
`(issuer, subject)` byte pair, which may be either kind of principal, and contain a
subset of:
...
Result tabs load and mount on first use. Opening a Run initially reads Artifact metadata to decide whether Document or Structure is the useful first tab. Structure then reads its first Block page and Asset metadata, Layout reads Page metadata and the selected page's Blocks, and Resources reuses or reads Asset and Artifact metadata. A tab that has never been opened has no panel DOM, iframe, or image elements. Once opened, its component stays mounted while another tab is visible, preserving selection and scroll position without another metadata read; selecting a different Run unmounts every old panel and resets all tab state.

Metadata responses retain their public shapes, but the activated panels bound what they render locally. Layout exposes 50 Page choices at a time, Resources exposes 24 image Assets and 50 Artifacts per page, and only the current slices reach their templates. Pagination uses native buttons, preserves each position while moving among tabs in one Run, clamps positions if a collection changes, and resets with a different Run. Block, Layout, and Resource images use native lazy loading; Resource image elements exist only for the current Asset page, so unvisited pages cannot initiate content requests. This bounds result-derived DOM and image work, not the size of the Page, Asset, or Artifact metadata response itself.

Every result read captures both the current Parse Run and its selection lifecycle. Success, failure, and loading completion are applied only while that lifecycle still owns the screen. Selecting a second Parse Run while the first one's reads are in flight is one click, and without all three checks the slower answer can replace the newer panel, show a stale error, or clear its loading indicator.
...
A background pass always refreshes the Document list, including when no Document is selected. That list is what keeps polling alive for unfinished visible work and what stops it after the final state arrives. When a selection exists, the pass also captures the selected Document and Parse Run when it starts, requests that Document's Run list, and applies the answer only while the same Document remains selected. Opening another Document clears the previous Run list immediately; success or failure from the previous selection is then ignored. Background failures stay silent, because a service that is briefly unreachable should not clear the screen or interrupt someone reading a result, and the next pass is still scheduled. A refresh the user asked for still reports its failure. Result data only exists once a run reaches a final status, so that transition opens the selected Run; the tab lifecycle above then reads only the metadata needed for the visible panel rather than fetching every result collection on every pass.

This is polling rather than a push channel. It costs one Document-list request per interval per open workspace, plus one Run-list request when a Document is selected, against a stream that would have to survive a reverse proxy, a restart, and a lost connection to be worth the exchange at this scale.
