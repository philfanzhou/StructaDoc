...
本规格定义 StructaDoc 在 Provider、数据库、管理网页和公共 API 之间共享的统一结构化文档语义。它是完整目标契约；当前已实现 Document 持久化与读取，以及 Parse Bundle、Page、Block、Asset、Artifact 的验证、首个 MinerU 归一化器和幂等成功提交。统一结果的公共读取 DTO、下载端点和更多 Provider 输出版本仍未实现。
